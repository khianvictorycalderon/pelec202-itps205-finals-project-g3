...
function useFetch(indicator) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/${indicator}.json`)
      ...
      .then((j) =>
        setState({ data: j.value || [], loading: false, error: null }),
      )
...
export default function Statistics() {
  ...
  const bmiChart = (() => {
    ...
    const byCountry = groupAvg(atLatest, (d) => d.SpatialDim)
      ...
      .slice(0, 10);
    const globalAvg = +(
      atLatest.reduce((s, d) => s + d.NumericValue, 0) / atLatest.length
    ).toFixed(1);
    const countryCount = new Set(atLatest.map((d) => d.SpatialDim)).size;
    return {
      year: latest,
      labels: byCountry.map((d) => d.key),
      values: byCountry.map((d) => d.avg),
      globalAvg,
      countryCount,
    };
  })();
  ...
  const neonatalChart = (() => {
    ...
    if (!rows.length) return null;
    const byYear = groupAvg(rows, (d) => d.TimeDim).sort(
      (a, b) => Number(a.key) - Number(b.key),
    );
    const latest = byYear.at(-1);
    const oldest = byYear[0];
    const change =
      latest && oldest ? +(latest.avg - oldest.avg).toFixed(1) : null;
    return {
  ...
  const airChart = (() => {
    ...
    return {
      year: latest,
      labels: regions.map((r) => r.key),
      values: regions.map((r) => r.val),
    };
  })();
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-neutral-500 text-sm mt-2 max-w-xl">
            Real-time data from the WHO Global Health Observatory across four
            key indicators.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          ...
          <StatPill
            ...
            value={
              neonatalChart?.change != null
                ? `${neonatalChart.change > 0 ? "+" : ""}${neonatalChart.change}`
                : "…"
            }
            accent={changeAccent}
          />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <ChartCard
            ...
            subtitle={
              bmiChart
                ? `Top 10 countries · ${bmiChart.year} · avg of male & female (%)`
                : "Loading…"
            }
            loading={bmi.loading}
            error={bmi.error}
                  ...
                  datasets: [
                    {
                      ...
                      backgroundColor: bmiChart.labels.map(
                        (_, i) => `hsl(${185 + i * 7}, 68%, ${54 - i * 2}%)`,
                      ),
                      borderRadius: 6,
                    },
                  ],
          ...
          <ChartCard
            ...
            subtitle={
              glucoseChart
                ? `Top 10 countries · ${glucoseChart.year} · SEX_BTSX (%)`
                : "Loading…"
            }
                  ...
                  datasets: [
                    {
                      ...
                      backgroundColor: glucoseChart.labels.map(
                        (_, i) => `hsl(${250 + i * 6}, 65%, ${58 - i * 2}%)`,
                      ),
                      borderRadius: 6,
                    },
                  ],
          ...
          <ChartCard
            ...
            subtitle={
              airChart
                ? `By WHO region · ${airChart.year} · SEX_BTSX · summed across age groups`
                : "Loading…"
            }
            loading={air.loading}
            error={air.error}
                  ...
                  datasets: [
                    {
                      ...
                      backgroundColor: REGION_PALETTE.slice(
                        0,
                        airChart.labels.length,
                      ),
                      borderWidth: 0,
                      cutout: "62%",
                    },
